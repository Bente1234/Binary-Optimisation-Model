"""
Storage Location Optimisation Heuristic
This script implements a greedy heuristic for assigning tools to storage sites 
based on cost and service-level constraints. 

All data used in the original industrial case are confidential and therefore 
excluded; this script contains only the model logic for educational use.
"""
# Each tool has attributes:
# sr/sp/se  – Service Level Agreements thresholds for routine, priority, emergency requests
# f         – tool usage frequency
# fp        – footprint (storage area requirement)
# m_cost    – maintenance cost per tool
...
for t in tools_df.index:
    ...
    # Phase 1 – Evaluate all candidate locations and compute unconstrained costs
    ...
    # Phase 2 – Enforce service-level constraints and assign top-ranked tools
    if sla_opts:
...
# Phase 3 – Allocate remaining tools while respecting capacity limits
for r in records:
